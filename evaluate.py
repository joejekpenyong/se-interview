...
print(f"\nFound {len(llm_spans)} LLM spans to evaluate")
...
# ── Eval 2: Tool Selection Correctness ───────────────────────────────────────
...
def prepare_eval_df(results_df, llm_spans_df):
    """Prepare evaluation dataframe with span_id as index."""
    eval_df = results_df[["label", "explanation"]].copy()
    eval_df.index = llm_spans_df["context.span_id"].values
    eval_df.index.name = "span_id"
    return eval_df

frustration_df = prepare_eval_df(frustration_results, llm_spans)
tool_selection_df = prepare_eval_df(tool_selection_results, llm_spans)
completeness_df = prepare_eval_df(completeness_results, llm_spans)
